...
for i,result_args in results_df.iterrows():
    ...
    y_multi_train, y_multi_test = y_multi[:trainlen], y_multi[trainlen:trainlen+testlen]
    ...
    #Predicting Train
    y_approx = n_predict(model,X_train, batch_size=arg.batch_size)
    for i in range(12):
        y_approx[:,i] = preproc_out.inverse_transform(y_approx[:,i].reshape((-1,1))).reshape((-1))
    score_train = metrics.mean_squared_error(y_multi_train,y_approx)

    #Predicting Test
    y_test_approx = n_predict(model,X_test, batch_size= arg.batch_size)
    for i in range(12):
        y_test_approx[:,i] = preproc_out.inverse_transform(y_test_approx[:,i].reshape((-1,1))).reshape((-1))
    score_test = metrics.mean_squared_error(y_multi_test,y_test_approx)

    print("Score train:", score_train, " Score test:", score_test )

    base = "../results/best_lstm_one/"
    ...
    np.savetxt(base + "lstm_y_train_{}_{}_lags_{}_timesteps.csv".format(source,input_dim, result_args.timesteps),y_multi_train)
    np.savetxt(base + "lstm_y_test_{}_{}_lags_{}_timesteps.csv".format(source,input_dim, result_args.timesteps),y_multi_test)
